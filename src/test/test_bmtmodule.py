...
def test_logged_phase_decorator(additionals):
    # logged_phase decorator should only work on methods that are of a class that inherits from BMTModule
    class BadClass:
        @BMTModule.logged_phase("somekey")
        def improper_func(self):
            pass

    class GoodClass(BMTModule):
        @BMTModule.logged_phase("somekey")
