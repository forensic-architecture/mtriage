...
def scaffold_empty(selname, elements=[], analysers=[]):
    ...
    if not os.path.exists(TEMP_ELEMENT_DIR):
        raise Exception("temp element dir doesn't exist - you need to instantiate it in a selector or analyser before using test methods.")
    ...
    for element in elements:
        ...
        if len(analysers)>0:
            for analyser in analysers:
                os.makedirs(f"{TEMP_ELEMENT_DIR}/{selname}/{Analyser.DERIVED_EXT}/{analyser}/{element}")

def get_element_path(selname, elementId, analyser=None):
    middle_insert = Analyser.DATA_EXT if analyser is None else f"{Analyser.DERIVED_EXT}/{analyser}"
