...
class TestSelectorErrors(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.indexErrorSelector = ErrorThrowingSelector(indexConfig, indexModule, TEMP_ELEMENT_DIR)

        retrieveModule = "retrieveErrorSelector"
        retrieveConfig = {}
        self.retrieveErrorSelector = ErrorThrowingSelector(retrieveConfig, retrieveModule, TEMP_ELEMENT_DIR)

        scaffold_empty(retrieveModule, elements=["skip","retry3","retryN","pass"])
    ...
    def test_integration(self):
        ...
        path = get_element_path("retrieveErrorSelector", "skip")
        self.assertFalse(os.exists(path))
